...
serve(async (req) => {
  try {
    ...
    const authMatches = token === REVENUECAT_SECRET;
    ...
    if (
      !event ||
      !['INITIAL_PURCHASE', 'NON_RENEWING_PURCHASE', 'RENEWAL'].includes(
        event.type,
      )
    ) {
    ...
    console.log(
      `Processing purchase: User=${userId}, Product=${productId}, Tx=${transactionId}`,
    );
    ...
    console.log(
      `Successfully credited ${coinAmount} coins. New balance: ${result.new_balance}`,
    );
    return new Response(
      JSON.stringify({
        success: true,
