...
export const moderationService = {
  ...
  createReport: async (data: ReportRequest): Promise<{ report: Report }> => {
    try {
      ...
      const { data: reportData, error } = await dbModerationService.createReport({
        reporter_id: user.id,
  ...
  getMyReports: async (params?: {
    ...
    try {
      ...
      const { data, count, error } = await dbModerationService.listReports(user.id);
      if (error) throw error;
  ...
  getBlockedUsers: async (): Promise<{ blockedUsers: BlockedUser[] }> => {
    try {
      ...
      const { data, error } = await dbModerationService.listBlockedUsers(user.id);
      if (error) throw error;
