...
> **Important:** During the January 2026 code cleanup, placeholder implementations for
> `sslPinning.ts` and `deviceIntegrity.ts` were intentionally removed. These files contained
> non-functional code that was never integrated into the production build, creating a false sense of
> security.
>
> **Before Production Release with Financial Transactions:**
>
> - `react-native-ssl-pinning` MUST be implemented for Supabase API calls
> - Google Play Integrity API and Apple DeviceCheck MUST be integrated
> - These are **blocking requirements** for payment/wallet features
>
> The original placeholder code can be restored from git history if needed:
>
> ```bash
> git show efcb840~1:apps/mobile/src/utils/sslPinning.ts > sslPinning.ts
> git show efcb840~1:apps/mobile/src/utils/deviceIntegrity.ts > deviceIntegrity.ts
> ```
...
**Priority:** 🔴 CRITICAL (Pre-Production for Financial Features)  
**Status:** Not Implemented  
**Estimated Effort:** 2-3 days

**Problem:** Currently, the app trusts any valid SSL certificate signed by a CA. This makes it
vulnerable to:
