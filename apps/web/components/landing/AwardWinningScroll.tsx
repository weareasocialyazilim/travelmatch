...
import { Gift, Heart, Zap, Star } from 'lucide-react';

interface AwardWinningScrollProps {
  children: React.ReactNode;
}

export function AwardWinningScrollWrapper({ children }: AwardWinningScrollProps) {
...
function StickyMiniPortal({ scrollProgress }: { scrollProgress: MotionValue<number> }) {
  // Transform values
  const scale = useTransform(scrollProgress, [0, 0.1], [0, 1]);
  const opacity = useTransform(scrollProgress, [0, 0.05, 0.1, 0.9, 1], [0, 0, 0.8, 0.8, 0]);
...
function FloatingGiftParticles({ scrollProgress }: { scrollProgress: MotionValue<number> }) {
