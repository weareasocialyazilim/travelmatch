...
function ScrollGiftParticles({
  ...
  const particles = Array.from({ length: 20 }, (_, i) => ({
    ...
    duration: 4 + seededRandom(i * 5 + 3) * 3,
    size: 6 + seededRandom(i * 5 + 4) * 10,
    type: types[Math.floor(seededRandom(i * 5 + 5) * 4)],
  }));
    ...
    >
      {particles.map((particle) => (
        ...
        >
          ...
          {particle.type === 'star' && (
            <Star size={particle.size} fill="currentColor" />
          )}
        </motion.div>
      ))}
    </motion.div>
...
export function GravityContent({
  children,
  className = '',
  intensity = 1,
}: GravityContentProps) {
  ...
  const rotateX = useSpring(
    useTransform(mouseY, [-0.5, 0.5], [5 * intensity, -5 * intensity]),
    {
      stiffness: 100,
      damping: 30,
    },
  ...
  const rotateY = useSpring(
    useTransform(mouseX, [-0.5, 0.5], [-5 * intensity, 5 * intensity]),
    {
      stiffness: 100,
      damping: 30,
    },
...
export function LiquidTransitionDivider({
  fromColor: _fromColor = 'black',
  toColor = 'purple-950',
}) {
  void _fromColor;
  return (
