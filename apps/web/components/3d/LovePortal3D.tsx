...
function LiquidPortalMesh({
  ...
  useFrame((state) => {
    ...
    if (
      ...
      materialRef.current?.uniforms?.uMouse
    ) {
...
function AmbientParticles() {
  ...
  const [positions, colors] = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      // Distribute in a sphere around the portal using seeded random
      const theta = seededRandom(i * 3 + 1) * Math.PI * 2;
      const phi = Math.acos(2 * seededRandom(i * 3 + 2) - 1);
      const radius = 2 + seededRandom(i * 3 + 3) * 3;

      positions[i * 3] = radius * Math.sin(phi) * Math.cos(theta);
      positions[i * 3 + 1] = radius * Math.sin(phi) * Math.sin(theta);
      positions[i * 3 + 2] = radius * Math.cos(phi);

      // Color between orchid and gold using seeded random
      const mixRatio = seededRandom(i * 3 + 4);
