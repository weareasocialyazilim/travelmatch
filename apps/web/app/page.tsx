...
const CONTENT = {
  en: {
    ...
    hero: {
      ...
      desc: (
        <>
          Virtual gifts are <span className="highlight-text">CRINGE.</span>
          <br />
          Send a real coffee to someone in Paris right now. Distance is just a
          number, the experience is universal. 🧢
        </>
      ),
      ctaStart: 'START NOW',
      ctaDemo: 'WATCH DEMO',
  ...
  tr: {
    ...
    hero: {
      ...
      desc: (
        <>
          Sanal hediyeler <span className="highlight-text">KRİNÇ.</span>
          <br />
          Şu an Paris&apos;teki birine gerçek bir kahve ısmarla. Mesafe sadece
          bir sayı, deneyim evrenseldir. 🧢
        </>
      ),
      ctaStart: 'BAŞLA',
      ctaDemo: 'DEMOYU İZLE',
...
const Hero = ({
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute bottom-[20%] right-[10%] rotate-[6deg] z-10 animate-float"
    ...
    <div className="w-full h-full relative transition-all duration-500 group-hover:-translate-y-4 group-hover:rotate-1">
      ...
      <div className="absolute inset-0 bg-black border-4 border-white rounded-[30px] overflow-hidden z-10 flex flex-col">
        ...
        <div className="p-6 bg-black border-t-4 border-white relative">
          <div
            className={`absolute -top-5 left-6 inline-block px-4 py-1 border-2 border-black transform -skew-x-12 shadow-[4px_4px_0px_rgba(0,0,0,0.5)] ${item.color === 'pink' ? 'bg-[#FF0099] text-white' : 'bg-[#00F0FF] text-black'}`}
          >
      ...
      {/* Retro Shadow Layer */}
      <div
        className={`absolute top-5 left-5 w-full h-full rounded-[30px] border-4 border-white z-0 ${item.color === 'pink' ? 'bg-[#FF0099]' : 'bg-[#00F0FF]'}`}
      ></div>
    </div>
  </div>
);
...
const TheStash = ({
  ...
  const items: GiftItem[] = [
    ...
    },
    {
      ...
      color: 'pink',
      user: '@mike',
      userImg: 'https://i.pravatar.cc/150?u=50',
        ...
        <div className="flex flex-col md:flex-row justify-center gap-10 mb-24">
          {['APP STORE', 'PLAY STORE'].map((store, i) => (
            <div key={i} className="relative group w-full md:w-auto">
              ...
              <div
                className={`absolute -top-4 -right-4 ${i === 0 ? 'bg-[#CCFF00] text-black' : 'bg-[#FF0099] text-white'} text-xs font-black font-mono py-1.5 px-4 border-2 border-black transform ${i === 0 ? 'rotate-6' : '-rotate-6'} shadow-[4px_4px_0px_#000] z-20 pointer-events-none`}
              >
      ...
      {menuOpen && (
        <div className="fixed inset-0 bg-[#FF0099] z-[100] flex flex-col items-center justify-center animate-[popup-in_0.3s_ease-out]">
          ...
          <div className="flex flex-col gap-6 text-center">
            {[c.nav.features, c.stash.titleEnd, c.nav.manifesto, 'CONTACT'].map(
              (item) => (
                <button
                  key={item}
                  onClick={() => {
