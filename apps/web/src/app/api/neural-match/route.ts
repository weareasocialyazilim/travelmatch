...
const INTEREST_MAPPING: Record<string, string[]> = {
  ...
  'human connection': ['social', 'community', 'gathering', 'meeting', 'bonding'],
...
function transformMLResponse(
  ...
  const recommendations = (mlData.suggestions || mlData.recommendations || []) as Array<{
    id?: string;
    title?: string;
    name?: string;
    score?: number;
  ...
  return {
    ...
    recommendations: recommendations.slice(0, 4).map((rec, i) => ({
      ...
      score: Math.round((rec.score || rec.matchScore || 0.7 + Math.random() * 0.3) * 100),
...
function calculateOverallScore(recommendations: Array<{ score?: number; matchScore?: number }>): number {
