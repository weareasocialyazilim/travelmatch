...
export async function POST(req: Request) {
  try {
    ...
    if (seoResult.status === 'cycle_complete') {
      ...
      // Log SEO update
      console.log('[SEO Auto-Pilot] Cycle complete:', {
        keywords: seoResult.totalKeywords,
        vibe: seoResult.currentVibe,
        topTrend: seoResult.topTrends[0]?.keyword,
      });
    }
    ...
    console.error('SEO auto-pilot error:', error);
    return NextResponse.json(
