...
/**
 * Proof Page - Web fallback for shared proof links
 *
 * URL: https://www.lovendo.xyz/proofs/{id}
 *
 * This page serves as a web fallback when users share proof links.
 * On mobile devices with the app installed, users will be redirected
 * via the app's intent filters (lovendo://proof/{id}).
 */
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { id } = await params;
  return {
    title: 'Proof | Lovendo',
    description: 'View this proof on Lovendo',
    alternates: {
      canonical: `https://www.lovendo.xyz/proofs/${id}`,
    },
  };
}
...
export default async function ProofPage({ params }: PageProps) {
  ...
  // In production, fetch proof data from Supabase
      ...
      <div className="max-w-md w-full text-center">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-6">
          ...
          <p className="text-[#666] mb-6">
            This proof is waiting for you. Download the Lovendo app to view and verify.
          </p>
