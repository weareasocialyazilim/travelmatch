  ...
  return useQuery<AnalyticsChartData>({
    ...
    queryFn: async () => {
      ...
      if (error) {
        ...
        const total =
          Array.from(sourceMap.values()).reduce((a, b) => a + b, 0) || 1;
        ...
        const processedAcquisition: UserAcquisitionData[] = Array.from(
          sourceMap.entries(),
        ).map(([source, count]) => ({
          source,
        ...
        const totalCityUsers =
          Array.from(cityMap.values()).reduce((a, b) => a + b, 0) || 1;
        ...
        const processedContentStats: ContentStatData[] = [
          {
            metric: 'Toplam Moment',
            value: formatNumber(totalMoments || 0),
            change: '+12%',
            trend: 'up',
          },
          {
            metric: 'Günlük Paylaşım',
            value: String(todayMoments || 0),
            change: '+8%',
            trend: 'up',
          },
          {
            metric: 'Ort. Etkileşim',
            value: '23.4',
            change: '-3%',
            trend: 'down',
          },
          {
            metric: 'Onay Oranı',
            value: `${totalMoments ? Math.round(((approvedMoments || 0) / totalMoments) * 100) : 0}%`,
            change: '+2%',
            trend: 'up',
          },
        ];
        ...
        return {
          dailyActiveUsers:
            processedDau.length > 0
              ? processedDau
              : mockAnalyticsData.dailyActiveUsers,
          revenueData:
          ...
          userAcquisition:
            processedAcquisition.length > 0
              ? processedAcquisition
              : mockAnalyticsData.userAcquisition,
  ...
  useEffect(() => {
    if (chartError) {
      toast({
        ...
        description:
          'Grafik verileri yüklenemedi. Varsayılan veriler gösteriliyor.',
        variant: 'destructive',
      });
    }
    if (metricsError) {
    ...
    <div className="space-y-6">
      ...
      {isPlaceholderData && !isLoading && (
        <div className="flex items-center gap-2 rounded-lg border border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950 px-4 py-2 text-sm text-amber-700 dark:text-amber-300">
          ...
          <span>
            Gerçek zamanlı veriler yüklenemedi. Varsayılan veriler gösteriliyor.
          </span>
        </div>
      )}
