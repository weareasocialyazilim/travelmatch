...
export default function CEOBriefingPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <Card className="admin-card col-span-1 bg-gradient-to-br from-primary/5 to-primary/10 border-primary/20">
          <CardContent className="p-6 text-center">
            <p className="text-sm text-muted-foreground mb-2">Şirket Sağlık Skoru</p>
        ...
        <Card className="admin-card col-span-2">
          ...
          <CardContent>
            <div className="flex items-end gap-4">
              <div>
                <p className="text-5xl font-bold">{northStar.current.toLocaleString()}</p>
                <div className="flex items-center gap-2 mt-1">
                  {northStar.trend === 'up' ? (
                    <Badge className="bg-green-500/10 text-green-500">
                      <ArrowUpRight className="h-3 w-3 mr-1" />
                      +{((northStar.current - northStar.lastWeek) / northStar.lastWeek * 100).toFixed(1)}%
                    </Badge>
                  ) : (
                  ...
                  <span className="text-sm text-muted-foreground">vs geçen hafta</span>
                </div>
              </div>
              <div className="flex-1">
                <div className="flex justify-between text-sm mb-1">
                  <span>Hedefe ilerleme</span>
                  <span className="font-medium">{northStar.percentToGoal}%</span>
                </div>
          ...
          <CardContent className="space-y-3">
            {todayEvents.map((event, i) => (
              <div key={i} className="flex items-center gap-3 p-2 rounded-lg bg-muted/30">
                <span className="text-sm font-mono font-medium w-12">{event.time}</span>
                <div className="flex-1">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: 'Escrow Dashboard', href: '/escrow-operations', icon: DollarSign, count: 245 },
          { label: 'Proof Queue', href: '/proof-center', icon: CheckCircle2, count: 47 },
          { label: 'Safety Alerts', href: '/safety-hub', icon: Shield, count: 12 },
          { label: 'System Health', href: '/system-health', icon: Activity, count: null },
        ].map((link) => (
          <Card key={link.label} className="admin-card hover:border-primary/50 cursor-pointer transition-colors">
