...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: logs, count, error } = await (supabase.from('audit_logs') as any)
      .select('*', { count: 'exact' })
    ...
    const formattedHistory: LoginHistoryEntry[] = (logs || []).map((log: Record<string, unknown>) => ({
      id: log.id,
      status: getStatusFromAction(log.action as string),
      ip_address: log.ip_address || 'Unknown',
      location: log.location || getLocationFromIP(log.ip_address as string),
      device: log.user_agent ? parseUserAgent(log.user_agent as string) : 'Unknown',
      created_at: log.created_at,
      reason: getReasonFromAction(log.action as string, log.details as Record<string, unknown>),
    }));

    return NextResponse.json({
      history: formattedHistory.length > 0 ? formattedHistory : getMockLoginHistory(),
 ...
 */
function getReasonFromAction(action: string, details?: Record<string, unknown>): string | undefined {
