...
export function MobileHealthInbox() {
  ...
  const getStatusIcon = (status: 'ok' | 'missing' | 'invalid' | null) => {
    ...
    if (status === 'missing' || status === 'invalid') return <XCircle className="h-3 w-3 text-red-400" />;
          ...
          {parsed && (
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                <span className="text-[10px] text-slate-500 uppercase tracking-wider">Triage</span>
              ...
              <div className="p-2 rounded-lg bg-slate-800/30 border border-slate-700/30">
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
                  ...
                  <span className="text-slate-200">
                    ...
                    {parsed.buildInfo.buildNumber && ` (${parsed.buildInfo.buildNumber})`}
                  </span>
                  <span className="text-slate-400">Platform</span>
                  <span className="text-slate-200">{parsed.buildInfo.platform || 'N/A'}</span>
                  <span className="text-slate-400">Device</span>
                  <span className="text-slate-200">{parsed.buildInfo.device || 'Unknown'}</span>
                  <span className="text-slate-400">Environment</span>
                  <span className={cn(
                    'capitalize',
                    parsed.buildInfo.environment === 'production' && 'text-emerald-400',
                    parsed.buildInfo.environment === 'staging' && 'text-amber-400',
                    parsed.buildInfo.environment === 'development' && 'text-blue-400'
                  )}>
                ...
                <div className="space-y-1 text-xs">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      <span className="text-slate-200">{parsed.configSanity.supabaseUrl || 'N/A'}</span>
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      <span className="text-slate-200">{parsed.configSanity.supabaseKey || 'N/A'}</span>
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <span className={cn(
                      ...
                      parsed.configSanity.serviceRoleLeak ? 'text-red-400' : 'text-emerald-400'
                    )}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-slate-200">{parsed.configSanity.authState || 'N/A'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-2 rounded-lg bg-slate-800/30 border border-slate-700/30">
                ...
                <p className={cn(
                  ...
                  parsed.errorCount >= 5 && parsed.errorCount < 20 && 'text-amber-400',
                ...
                <div className="p-2 rounded-lg bg-slate-800/30 border border-slate-700/30">
                  ...
                  <div className="space-y-1">
                    {parsed.slowScreens.map((screen) => (
                      <div key={screen.screenName} className="flex items-center justify-between text-xs">
                        ...
                        <span className={cn(
                          ...
                          screen.avgTtiMs > 1000 && screen.avgTtiMs <= 2000 && 'text-amber-400',
                          screen.avgTtiMs <= 1000 && 'text-emerald-400'
                        )}>
                          {screen.avgTtiMs}ms
                          <span className="text-slate-500 ml-1">({screen.count}x)</span>
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              {parsed.generatedAt && (
                <p className="text-[10px] text-slate-500 text-center">
                  Generated: {new Date(parsed.generatedAt).toLocaleString('tr-TR')}
                </p>
              )}
