...
import { TrendingUp, TrendingDown, CreditCard, Send, MoreHorizontal, Info, AlertTriangle, CheckCircle2 } from 'lucide-react';
    ...
    <div className={cn(cardClass, className)}>
      ...
      {barData.length > 0 && (
        <div className="bar-chart-grid">
          {barData.map((val, i) => (
            <div key={i} className={barClass} style={{ height: `${(val / maxBar) * 100}%` }} />
          ))}
        </div>
      )}
    </div>
...
export function CreditCardWidget({ balance, cardNumber, cardHolder, className }: CreditCardWidgetProps) {
...
export function QuickTransferWidget({ recipients = [], onTransfer, className }: QuickTransferWidgetProps) {
  ...
  return (
    <Card className={cn('transfer-widget', className)}>
      ...
      <CardContent className="space-y-4">
            ...
            {recipients.map((r) => (
              <button
                ...
                className={cn(
                  ...
                  selectedId === r.id ? 'bg-primary/10 ring-2 ring-primary' : 'hover:bg-muted/50'
                )}
              >
                <img src={r.avatar} alt={r.name} className="w-10 h-10 rounded-full object-cover" />
                <span className="text-xs text-muted-foreground">{r.name.split(' ')[0]}</span>
              </button>
            ))}
          </div>
        )}
        ...
        <Button className="transfer-button w-full" onClick={() => onTransfer?.(amount, selectedId)}>
...
export function DailyLimitWidget({ used, limit, currency = '₺', className }: DailyLimitWidgetProps) {
      ...
      <CardContent>
        ...
        <div className="flex justify-between mt-3 text-sm">
          <span className="text-muted-foreground">
            Kullanılan: <span className="text-foreground font-medium">{currency}{used.toLocaleString()}</span>
          </span>
          <span className="text-muted-foreground">
            Kalan: <span className="text-emerald-500 font-medium">{currency}{remaining.toLocaleString()}</span>
          </span>
        </div>
      </CardContent>
    </Card>
...
export function NotificationPanel({
  notifications,
  tabs = [{ id: 'all', label: 'Tümü' }, { id: 'unread', label: 'Okunmamış' }],
  activeTab = 'all',
  onTabChange,
  onNotificationClick,
  className,
    ...
    <Card className={cn('notification-panel', className)}>
      <div className="notification-panel-tabs">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            className={cn('notification-panel-tab', activeTab === tab.id && 'notification-panel-tab-active')}
      ...
      <div className="notification-list">
        {notifications.length === 0 ? (
          ...
          notifications.map((n) => (
            <div
              key={n.id}
              className={cn('notification-item', !n.read && 'notification-item-unread')}
...
export function TrendIndicator({ title, value, trend, sparkline = [], className }: TrendIndicatorProps) {
  ...
  return (
    <Card className={cn('p-4', className)}>
      ...
      <div className="flex items-baseline gap-2 mt-1">
        ...
        <div className={cn('flex items-center gap-0.5 text-xs font-medium', trend >= 0 ? 'text-emerald-500' : 'text-rose-500')}>
      ...
      {sparkline.length > 0 && (
        <div className="flex items-end gap-0.5 h-10 mt-3">
          {sparkline.map((val, i) => (
            <div key={i} className="flex-1 rounded-t-sm bg-primary/30" style={{ height: `${(val / maxVal) * 100}%` }} />
          ))}
        </div>
      )}
    </Card>
