...
export function parseDiagnosticsSummary(text: string): ParsedDiagnostics {
  ...
  if (versionMatch) {
    ...
    result.buildInfo.buildNumber =
      versionMatch[2] !== 'N/A' ? versionMatch[2] : null;
  }
  ...
  if (supabaseUrlMatch) {
    result.configSanity.supabaseUrl = supabaseUrlMatch[2] as
      | 'ok'
      | 'missing'
      | 'invalid';
  }
  ...
  const serviceRoleMatch = text.match(
    /Service Role Leak:\s*(🚨 DETECTED|✅ None)/,
  );
  if (serviceRoleMatch) {
    result.configSanity.serviceRoleLeak =
      serviceRoleMatch[1].includes('DETECTED');
  }
  ...
  if (result.configSanity.serviceRoleLeak) {
    triage.push({
      level: 'P0',
      reason:
        'CRITICAL: Service role key leak detected! Immediate security response required.',
    });
  }

  if (
    result.configSanity.supabaseUrl === 'missing' ||
    result.configSanity.supabaseKey === 'missing'
  ) {
    triage.push({
      level: 'P0',
  ...
  const slowScreens = result.slowScreens.filter(
    (s) => s.avgTtiMs > 1000 && s.avgTtiMs <= 2000,
  );
  ...
  if (
    result.buildInfo.environment === 'production' &&
    !result.buildInfo.buildNumber
  ) {
    triage.push({
      level: 'P3',
      reason:
        'INFO: Build number missing in production. Consider adding for traceability.',
    });
  }
