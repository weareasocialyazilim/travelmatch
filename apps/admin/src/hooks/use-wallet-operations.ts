  ...
  return useQuery({
    ...
    queryFn: async (): Promise<WalletStats> => {
      ...
      try {
        ...
        const processedAmount = processedData?.reduce((sum, p) => sum + p.amount, 0) || 0;
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<KYCVerification[]> => {
      ...
      try {
        ...
        toast.error('KYC doğrulamaları yüklenemedi, örnek veriler gösteriliyor');
    ...
    mutationFn: async ({
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          failure_reason: action === 'reject' ? 'Toplu işlemde reddedildi' : null,
        })
        .in('id', payoutIds)
        .select();
