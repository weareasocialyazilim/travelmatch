...
export interface FraudCase {
  ...
  type: 'payment_fraud' | 'identity_theft' | 'account_takeover' | 'fake_profile' | 'scam';
  reported_at: string;
...
export interface LinkedAccount {
  ...
  user_name: string;
  user_email: string;
  connection_type: 'same_ip' | 'same_device' | 'same_payment' | 'same_phone' | 'behavioral';
...
export function useAddFraudEvidence() {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ['fraud-evidence', data.case_id] });
