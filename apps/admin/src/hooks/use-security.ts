...
async function fetchLoginHistory(limit = 30, offset = 0): Promise<LoginHistoryResponse> {
  try {
    const params = new URLSearchParams();
    params.set('limit', limit.toString());
    params.set('offset', offset.toString());

    const response = await fetch(`/api/security/login-history?${params.toString()}`);
    if (!response.ok) {
    ...
    // Return mock data on error
    return { history: mockLoginHistory, total: mockLoginHistory.length, limit, offset };
  }
}
...
async function revokeSession(sessionId: string): Promise<{ success: boolean; message: string }> {
  const response = await fetch('/api/security/sessions', {
    method: 'DELETE',
...
async function revokeAllSessions(): Promise<{ success: boolean; message: string }> {
  const response = await fetch('/api/security/sessions', {
    method: 'DELETE',
    headers: { 'Content-Type': 'application/json' },
