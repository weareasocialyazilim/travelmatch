  ...
  return useRealtimeSubscription<RealtimeTransaction>({
    ...
    onUpdate: ({ new: tx }) => {
      if (tx.status === 'failed' || tx.status === 'refunded') {
        ...
        toast.error(`İşlem ${tx.status === 'failed' ? 'başarısız' : 'iade edildi'}`, {
...
export function useCommandCenterRealtime() {
  ...
  // Proofs
  useRealtimeProofs(
    () => setCounters(prev => ({ ...prev, pendingProofs: prev.pendingProofs + 1 })),
    (proof) => {
      if (proof.status !== 'pending') {
        setCounters(prev => ({ ...prev, pendingProofs: Math.max(0, prev.pendingProofs - 1) }));
      }
    },
  );
  ...
  // Disputes
  useRealtimeDisputes(
    () => setCounters(prev => ({ ...prev, activeDisputes: prev.activeDisputes + 1 })),
    (dispute) => {
      if (dispute.status === 'resolved' || dispute.status === 'dismissed') {
        setCounters(prev => ({ ...prev, activeDisputes: Math.max(0, prev.activeDisputes - 1) }));
      }
    },
  );
  ...
  useRealtimeTransactions({
    minAmount: 5000,
    onNewTransaction: () => setCounters(prev => ({ ...prev, highValueTx: prev.highValueTx + 1 })),
  });
