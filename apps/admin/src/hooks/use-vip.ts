...
async function fetchVIPUsersFromSupabase(
  ...
  const users: VIPUser[] = (data || []).map(
    (user: Record<string, unknown>) => ({
      id: user.id as string,
      user_id: user.id as string,
      tier: ((user.subscription_tier as string) || 'vip') as
        | 'vip'
        | 'influencer'
        | 'partner',
      ...
      user: {
        display_name:
          (user.display_name as string) || (user.full_name as string) || '',
