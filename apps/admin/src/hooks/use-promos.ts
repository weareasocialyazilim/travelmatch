...
async function fetchPromos(
  ...
  if (params.isActive !== undefined)
    searchParams.set('is_active', params.isActive.toString());
...
export function usePromosPageData() {
  ...
  const shouldUseMockPromos =
    ...
    (!promosQuery.isLoading &&
      (!promosQuery.data?.promo_codes ||
        promosQuery.data.promo_codes.length === 0));
  ...
  const promoStats = shouldUseMockPromos
    ...
    : {
        ...
        activeCodes:
          ...
          mockPromoStats.activeCodes,
        totalUsage:
          promoCodes.reduce((sum, p) => sum + (p.usage_count || 0), 0) ||
          mockPromoStats.totalUsage,
  ...
  return {
    ...
    error:
      promosQuery.isError && referralsQuery.isError ? promosQuery.error : null,
