...
export function logScreenTTI(screenName: string, ttiMs: number): void {
  try {
    ...
    const updatedLog = addToRingBuffer(currentLog, entry, MAX_PERFORMANCE_LOG_SIZE);
    storage.set(DIAGNOSTICS_STORAGE_KEYS.PERFORMANCE_LOG, JSON.stringify(updatedLog));
  } catch {
    // Silent fail
  }
}
...
export function getTopSlowScreens(limit: number = 5): Array<{
  ...
  for (const entry of log) {
    const existing = screenStats.get(entry.screenName) || { total: 0, count: 0 };
...
export function getBuildInfo(): DiagnosticsBuildInfo {
  ...
  return {
    appVersion: Application.nativeApplicationVersion || expoConfig?.version || '0.0.0',
