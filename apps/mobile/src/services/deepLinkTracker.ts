...
class DeepLinkTracker {
  ...
  async initialize() {
    ...
    logger.debug('[DeepLink] Tracker initialized');
  }
  ...
  private async handleDeepLink(url: string, isInitial: boolean) {
    try {
      ...
      logger.debug('[DeepLink] Tracked:', event.type, event.source);
    } catch (error) {
  ...
  async trackConversion(goal: ConversionGoal, metadata?: Record<string, any>) {
    ...
    logger.debug('[DeepLink] Conversion:', goal, this.currentEvent.timeToComplete + 'ms');
  }
  ...
  async trackDropOff(screenName: string, reason?: string) {
    ...
    logger.debug('[DeepLink] Drop-off:', screenName, reason);
  }
