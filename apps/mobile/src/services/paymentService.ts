...
export const paymentService = {
  ...
  setDefaultCard: (cardId: string) => securePaymentService.setDefaultCard(cardId),
  addBankAccount: (data: {
  ...
  requestWithdrawal: (amount: number, bankAccountId: string) =>
    securePaymentService.withdrawFunds({
      amount,
      currency: 'USD',
      bankAccountId,
    }).then(result => result),
  ...
  releaseEscrow: (escrowId: string) => securePaymentService.releaseEscrow(escrowId),
  refundEscrow: (escrowId: string, reason: string) =>
    securePaymentService.refundEscrow(escrowId, reason),
  getUserEscrowTransactions: () => securePaymentService.getUserEscrowTransactions(),
};
