...
class ImagePreloader {
  ...
  async prefetchMomentsImages(moments: Array<{ id: string; imageUrl: string }>) {
    ...
    logger.debug('[Preload] Prefetched', uris.length, 'moment images');
  }
  ...
  async prefetchNextPage(currentPage: number, fetchPage: (page: number) => Promise<any>) {
    ...
    if (queryClient.getQueryData(queryKey)) {
      logger.debug('[Preload] Page', nextPage, 'already cached');
      return;
    }
    ...
    try {
      ...
      logger.debug('[Preload] Prefetched page', nextPage);
    } catch (error) {
      logger.error('[Preload] Failed to prefetch page', nextPage, error);
    }
  }
  ...
  private async processQueue() {
    ...
    while (this.queue.length > 0 && this.preloading.size < this.maxConcurrent) {
      ...
      this.preloadImage(item.uri)
        ...
        .catch((error) => {
          logger.error('[Preload] Failed to preload:', item.uri, error);
          this.preloading.delete(item.uri);
          this.processQueue(); // Process next item
        });
    }
  ...
  private async preloadImage(uri: string): Promise<void> {
    return new Promise((resolve, reject) => {
      Image.prefetch(uri)
        .then(() => {
          logger.debug('[Preload] Loaded:', uri);
          resolve();
        })
        .catch(reject);
    });
