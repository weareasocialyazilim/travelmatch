...
export interface DiagnosticsSummary {
  ...
  topSlowScreens: Array<{ screenName: string; avgTtiMs: number; count: number }>;
  generatedAt: string;
}
...
export const PII_PATTERNS = {
  ...
  phone: /(\+?[0-9]{1,4}[-.\s]?)?(\([0-9]{1,4}\)[-.\s]?)?[0-9]{1,4}[-.\s]?[0-9]{1,4}[-.\s]?[0-9]{1,9}/g,
  token: /eyJ[a-zA-Z0-9_-]*\.eyJ[a-zA-Z0-9_-]*\.[a-zA-Z0-9_-]*/g, // JWT
  uuid: /[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/gi,
  apiKey: /(key|token|secret|password|api_key|apikey)[=:]\s*['"]?[a-zA-Z0-9_-]{16,}['"]?/gi,
};
