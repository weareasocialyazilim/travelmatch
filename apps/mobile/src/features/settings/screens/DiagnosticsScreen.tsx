  ...
  return (
    ...
    >
      <MaterialCommunityIcons
        ...
        size={12}
        color={isOk ? COLORS.mint : isMissing ? COLORS.feedback.error : COLORS.feedback.warning}
      />
      {label && (
        <Text
          style={[
...
const ErrorLogItem: React.FC<{ entry: DiagnosticsErrorEntry }> = ({ entry }) => {
  ...
  const renderOverview = () => (
    <>
      ...
      <SectionCard title="Build Info" icon="information">
        <InfoRow label="Version" value={`${buildInfo.appVersion} (${buildInfo.buildNumber || 'N/A'})`} />
        ...
        {buildInfo.commitHash && (
          <InfoRow label="Commit" value={buildInfo.commitHash.substring(0, 8)} />
        )}
      </SectionCard>
      ...
      <SectionCard title="Config Sanity" icon="cog-outline">
        ...
        <InfoRow
          ...
          status={configSanity.authState === 'logged_in' ? 'ok' : configSanity.authState === 'logged_out' ? 'missing' : 'invalid'}
        />
      </SectionCard>
      ...
      <SectionCard title="Quick Stats" icon="chart-bar">
        ...
        <InfoRow
          ...
          value={
            errorLog.length > 0
              ? new Date(errorLog[errorLog.length - 1].timestamp).toLocaleString('tr-TR')
              : 'None'
          }
        />
        <InfoRow
  ...
  const renderPerformance = () => (
    <>
          ...
          topSlowScreens.map((screen, index) => (
            <View key={screen.screenName} style={styles.perfRow}>
              ...
              <View style={styles.perfInfo}>
                ...
                <Text style={styles.perfMeta}>{screen.count}x measurements</Text>
              </View>
      ...
      <View style={styles.perfLegend}>
        ...
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: COLORS.feedback.warning }]} />
          <Text style={styles.legendText}>Slow (1-2s)</Text>
        </View>
        <View style={styles.legendItem}>
          <View style={[styles.legendDot, { backgroundColor: COLORS.feedback.error }]} />
