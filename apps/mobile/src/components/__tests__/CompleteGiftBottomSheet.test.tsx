...
describe('CompleteGiftBottomSheet', () => {
  ...
  const getCardNumberInput = (component: ReturnType<typeof render>) => {
    ...
    return inputs.find(
      (input: { props: { placeholder?: string } }) =>
        input.props.placeholder === 'Card number',
    );
  };
  ...
  const getExpiryInput = (component: ReturnType<typeof render>) => {
    ...
    return inputs.find(
      (input: { props: { placeholder?: string } }) =>
        input.props.placeholder === 'Expiry',
    );
  };
  ...
  const getNameInput = (component: ReturnType<typeof render>) => {
    ...
    return inputs.find(
      (input: { props: { placeholder?: string } }) =>
        input.props.placeholder === 'Name on card',
    );
  };
  ...
  describe('Rendering', () => {
    it('renders when visible', () => {
      const { getByText } = render(
        <CompleteGiftBottomSheet {...defaultProps} />,
      );
    ...
    it('renders escrow notice', () => {
      ...
      expect(
        getByText('Your gift is held in escrow until the moment is verified.'),
      ).toBeTruthy();
    });

    it('does not render when not visible', () => {
      const { toJSON } = render(
        <CompleteGiftBottomSheet {...defaultProps} visible={false} />,
      );
      const modal = toJSON();
  ...
  describe('Edge Cases', () => {
    it('formats amount correctly with decimals', () => {
      const { getByText, rerender } = render(
        <CompleteGiftBottomSheet {...defaultProps} amount={100} />,
      );
    ...
    it('handles large amounts', () => {
      const { getByText } = render(
        <CompleteGiftBottomSheet {...defaultProps} amount={9999.99} />,
      );
    ...
    it('handles small amounts', () => {
      const { getByText } = render(
        <CompleteGiftBottomSheet {...defaultProps} amount={0.01} />,
      );
