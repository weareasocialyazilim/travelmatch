...
jest.mock('react-native-video', () => {
  const mockVideo = (props: any, ref: any) => {
    ...
    return React.createElement(View, { ...props, ref, testID: 'video-component' });
  };
  return mockVideo;
}, { virtual: true });
...
describe('AccessibleVideoPlayer', () => {
  ...
  describe('Rendering', () => {
    ...
    it('renders video component', () => {
      const { getByTestId } = render(<AccessibleVideoPlayer {...defaultProps} />);
    ...
    it('renders description when provided', () => {
      const { getByText } = render(
        <AccessibleVideoPlayer {...defaultProps} description="Test description" />
      );
    ...
    it('renders play/pause overlay button', () => {
      const { getByLabelText } = render(<AccessibleVideoPlayer {...defaultProps} />);
    ...
    it('does not render caption button when no captions provided', () => {
      const { queryByText } = render(<AccessibleVideoPlayer {...defaultProps} />);
    ...
    it('renders transcript button when transcript provided', () => {
      const { getByText } = render(
        <AccessibleVideoPlayer {...defaultProps} transcript="Test transcript" />
      );
  ...
  describe('Caption Functionality', () => {
    ...
    it('announces caption state changes', () => {
      ...
      expect(mockAnnounce).toHaveBeenCalledWith('Captions enabled', 'assertive');
    });
  ...
  describe('Playback Speed', () => {
    ...
    it('cycles through speed options', () => {
      const { getByLabelText, getByText } = render(<AccessibleVideoPlayer {...defaultProps} />);
    ...
    it('announces speed changes', () => {
      ...
      expect(mockAnnounce).toHaveBeenCalledWith('Playback speed: 1.25x', 'assertive');
    });
  });
  ...
  describe('Transcript Functionality', () => {
    it('transcript is hidden by default', () => {
      const { queryByText } = render(
        <AccessibleVideoPlayer {...defaultProps} transcript="Test transcript content" />
      );
    ...
    it('announces transcript visibility changes', () => {
      ...
      expect(mockAnnounce).toHaveBeenCalledWith('Transcript shown', 'assertive');
    });
  ...
  describe('Seek Controls', () => {
    ...
    it('announces rewind action', () => {
      ...
      expect(mockAnnounce).toHaveBeenCalledWith('Skipped backward 10 seconds', 'assertive');
    });
    ...
    it('announces forward action', () => {
      ...
      expect(mockAnnounce).toHaveBeenCalledWith('Skipped forward 10 seconds', 'assertive');
    });
  });
  ...
  describe('Accessibility', () => {
    ...
    it('caption button has proper selected state', () => {
      ...
      expect(enabledButton.props.accessibilityState).toEqual({ selected: true });
    });
    ...
    it('transcript button has proper selected state', () => {
      ...
      expect(shownButton.props.accessibilityState).toEqual({ selected: true });
    });
  });
  ...
  describe('Callbacks', () => {
    it('calls onProgress callback', async () => {
      ...
      await waitFor(() => {
        expect(mockOnProgress).toHaveBeenCalledWith({ currentTime: 30, duration: 120 });
      });
    });
  ...
  describe('Video Loading', () => {
    ...
    it('announces video duration when loaded', () => {
      ...
      expect(mockAnnounce).toHaveBeenCalledWith('Video loaded. Duration: 2:05', 'polite');
    });
  });
  ...
  describe('Edge Cases', () => {
    it('handles missing title gracefully', () => {
      const { getByTestId } = render(
        <AccessibleVideoPlayer videoUrl="https://example.com/video.mp4" title="" />
      );
    ...
    it('handles empty transcript', () => {
      ...
      // Should still show transcript panel with empty content
      expect(getByText('Transcript')).toBeTruthy();
    });
