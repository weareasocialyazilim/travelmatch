...
beforeAll(() => {
  (Linking as any).sendIntent = jest.fn(() => Promise.resolve());
});
...
describe('LowStorageAlert', () => {
  ...
  describe('Rendering - Critical Storage', () => {
    ...
    it('uses error color icon for critical storage', () => {
      const { UNSAFE_getAllByType } = render(<LowStorageAlert {...criticalProps} />);
  ...
  describe('User Interactions', () => {
    ...
    it('calls custom onOpenSettings when provided', () => {
      const { getByText } = render(
        <LowStorageAlert {...defaultProps} onOpenSettings={mockOnOpenSettings} />
      );
    ...
    it('calls onClose when modal backdrop is requested to close', () => {
      const { UNSAFE_getByType } = render(<LowStorageAlert {...defaultProps} />);
  ...
  describe('Platform-Specific Behavior', () => {
    it('opens iOS settings when no custom onOpenSettings provided (iOS)', () => {
      ...
      expect(Linking.openURL).toHaveBeenCalledWith('App-Prefs:root=General&path=iPhone Storage');
      expect(mockOnDismiss).toHaveBeenCalledTimes(1);
    });
    ...
    it('opens Android settings when no custom onOpenSettings provided (Android)', () => {
      ...
      expect(Linking.sendIntent).toHaveBeenCalledWith('android.settings.INTERNAL_STORAGE_SETTINGS');
      expect(mockOnDismiss).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('Modal Properties', () => {
    ...
    it('sets visible prop correctly', () => {
      const { UNSAFE_getByType, rerender } = render(<LowStorageAlert {...defaultProps} />);
  ...
  describe('Warning vs Critical Comparison', () => {
    it('shows different titles for warning vs critical', () => {
      const { getByText, rerender } = render(<LowStorageAlert {...defaultProps} level={StorageLevel.LOW} />);
      expect(getByText('Low Storage')).toBeTruthy();

      rerender(<LowStorageAlert {...defaultProps} level={StorageLevel.CRITICAL} />);
      expect(getByText('Storage Critical')).toBeTruthy();
    });
