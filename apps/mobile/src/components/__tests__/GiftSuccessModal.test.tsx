...
describe('GiftSuccessModal', () => {
  ...
  describe('Rendering', () => {
    ...
    it('displays formatted amount', () => {
      const { getByText } = render(<GiftSuccessModal {...defaultProps} amount={50} />);
      
      expect(getByText(/\$50\.00/)).toBeTruthy();
    });

    it('renders success icon', () => {
      const { UNSAFE_getAllByType } = render(<GiftSuccessModal {...defaultProps} />);
  ...
  describe('User Interactions', () => {
    ...
    it('calls onClose when modal backdrop is requested to close', () => {
      const { UNSAFE_getByType } = render(<GiftSuccessModal {...defaultProps} />);
      const modal = UNSAFE_getByType(require('react-native').Modal);
  ...
  describe('Modal Properties', () => {
    ...
    it('sets visible prop correctly', () => {
      const { UNSAFE_getByType, rerender } = render(<GiftSuccessModal {...defaultProps} />);
  ...
  describe('Optional Props', () => {
    ...
    it('renders with onViewApprovals provided', () => {
      const { getByText } = render(
        <GiftSuccessModal {...defaultProps} onViewApprovals={mockOnViewApprovals} />
      );
  ...
  describe('Amount Formatting', () => {
    it('formats whole numbers with decimals', () => {
      const { getByText } = render(<GiftSuccessModal {...defaultProps} amount={100} />);
      
      expect(getByText(/\$100\.00/)).toBeTruthy();
    });

    it('formats cents correctly', () => {
      const { getByText } = render(<GiftSuccessModal {...defaultProps} amount={0.99} />);
      
      expect(getByText(/\$0\.99/)).toBeTruthy();
    });

    it('formats decimal amounts', () => {
      const { getByText } = render(<GiftSuccessModal {...defaultProps} amount={25.5} />);
  ...
  describe('Edge Cases', () => {
    ...
    it('handles zero amount', () => {
      const { getByText } = render(<GiftSuccessModal {...defaultProps} amount={0} />);
      
      expect(getByText(/\$0\.00/)).toBeTruthy();
    });

    it('handles large amounts', () => {
      const { getByText } = render(<GiftSuccessModal {...defaultProps} amount={9999.99} />);
