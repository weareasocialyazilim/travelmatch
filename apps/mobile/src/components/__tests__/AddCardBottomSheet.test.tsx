...
describe('AddCardBottomSheet', () => {
  ...
  describe('Rendering', () => {
    ...
    it('renders security lock icon', () => {
      const { UNSAFE_getAllByType } = render(
        <AddCardBottomSheet {...defaultProps} />,
      );
      ...
      const lockIcon = icons.find(
        (icon: { props: { name: string } }) => icon.props.name === 'lock',
      );
    ...
    it('renders CVV help icon', () => {
      ...
      const helpIcon = icons.find(
        (icon: { props: { name: string } }) =>
          icon.props.name === 'help-circle-outline',
      );
  ...
  describe('Form Validation', () => {
    it('Add card button is disabled when card number is empty', () => {
      ...
      const addButton = touchables.find(
        (t: { props: { disabled?: boolean; children?: React.ReactNode } }) => {
          const children = t.props.children;
          return (
            children &&
            JSON.stringify(children).includes('Add card') &&
            t.props.disabled !== undefined
          );
        },
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('preserves form state when not saved', () => {
      ...
      const TouchableWithoutFeedback =
        require('react-native').TouchableWithoutFeedback;
      const touchables = component.UNSAFE_getAllByType(
