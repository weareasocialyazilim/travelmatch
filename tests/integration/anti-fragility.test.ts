...
// This ensures mockSupabase.from().select().single returns the same mock
...
// Mock Supabase client - from() always returns the same chain
const mockSupabase = {
  auth: {
    getUser: jest.fn(),
...
// Mock the supabase module using relative path
...
describe('Data Loading Anti-Fragility', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    // Reset the shared mock chain
    mockFromChain.select.mockClear().mockReturnThis();
    mockFromChain.eq.mockClear().mockReturnThis();
  ...
  it('should handle database connection error', async () => {
    mockSingleFn.mockRejectedValue(
  ...
  it('should handle query timeout', async () => {
    mockSingleFn.mockRejectedValue(new Error('Query timeout exceeded'));
  ...
  it('should handle malformed query', async () => {
    mockSingleFn.mockRejectedValue(new Error('Malformed query syntax'));
  ...
  it('should handle network interruption during query', async () => {
    mockSingleFn.mockRejectedValue(new Error('Network request failed'));
  ...
  it('should handle connection pool exhaustion', async () => {
    mockSingleFn.mockRejectedValue(new Error('Connection pool exhausted'));
