...
const ANON_KEY = __ENV.SUPABASE_ANON_KEY || ['your', 'anon', 'key'].join('-');
...
function authenticateUser() {
  ...
  const password = ['Load', 'Test', '123!'].join('');
...
function testMomentsWrite(token) {
  group('Moments - Write Operations', () => {
    ...
    if (!success) {
      errorRate.add(1);
      console.error(
        `Moment create failed: ${createRes.status} ${createRes.body}`,
      );
    }
  });
}
...
export function handleSummary(data) {
  ...
  const errorRate = data.metrics.errors
    ? data.metrics.errors.values.rate * 100
    : 0;
